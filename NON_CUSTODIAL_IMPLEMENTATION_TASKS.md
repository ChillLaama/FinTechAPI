...
Задачи:
- [ ] Удалить изменение `Account.balance` из `CreateTransactionAsync`.
- [ ] Удалить изменение `Account.balance` из `UpdateTransactionAsync`.
- [ ] Удалить изменение `Account.balance` из `UpdateTransactionStatusAsync`.
- [ ] Удалить изменение `Account.balance` из `DeleteTransactionAsync`.
- [ ] Удалить или перевести в legacy helper `GetBalanceDelta`.
- [ ] Обновить комментарии и поведение сервиса под business-state only модель.
...
Definition of Done:
- [ ] Ни одна операция над transaction больше не меняет внутренний balance.
- [ ] Сервис работает только со status lifecycle и metadata.
...
Задачи:
- [ ] Перестать использовать `Account.balance` как основной денежный атрибут.
- [ ] Определить новую семантику `Account`: profile / reporting bucket / logical grouping.
- [ ] Решить, оставляем ли поле `balance` физически как legacy на переходный период.
- [ ] Подготовить migration note по отказу от wallet semantics.
...
Задачи:
- [ ] Добавить endpoint `GET /api/platform/balance`.
- [ ] Определить DTO ответа: `available`, `pending`, `currency`, `source`, `syncedAt`.
- [ ] Реализовать сервис получения Stripe platform balance.
- [ ] Обработать ошибки Stripe и деградацию UI-friendly сообщениями.
...
Definition of Done:
- [ ] Баланс для UI можно получить только из Stripe-backed endpoint.
